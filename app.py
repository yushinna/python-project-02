...
def create_dict_team_and_players(players, teams):
    ...
    start = 0
    for team in teams:
        dict_team_and_players[team] = players[
            start: start + num_players_per_team
        ]
    ...
    while True:
        ...
        try:
            ...
            if index != '1' and index != '2' and index != '3':
                ...
                list_player_names = extract_player_names(
                    list_selected_team_and_players)
                num_inexperienced = count_num_inexperienced(
                    list_selected_team_and_players)
                num_experienced = num_players - num_inexperienced
                average_height = calculate_total_height(
                    list_selected_team_and_players) / num_players
                list_gurdian_names = extract_guardian_names(
                    list_selected_team_and_players)
                # print stat
                clear_screen()
...
if __name__ == "__main__":
    ...
    players_inexperienced, players_experienced = split_players_by_experience(
        updated_players)
    dict_team_and_players_inexperienced = create_dict_team_and_players(
        players_inexperienced, constants.TEAMS)
    dict_team_and_players_experienced = create_dict_team_and_players(
        players_experienced, constants.TEAMS)
    while True:
        clear_screen()
        print('\nBASKETBALL TEAM STATS TOOL\n')
        print('---- MENU----\n')
